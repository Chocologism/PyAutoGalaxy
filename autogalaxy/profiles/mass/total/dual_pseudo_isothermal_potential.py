...
class dPIEPotential(MassProfile):
    ...
    def _ellip(self):
        ...
        MAX_ELLIP = 0.99999
        return np.min(np.array([ellip, MAX_ELLIP]))
    ...
    def _deflection_angle(self, radii):
        ...
        f = radii / (a + np.sqrt(a**2 + radii**2)) - radii / (
            s + np.sqrt(s**2 + radii**2)
        )
    ...
    def deflections_yx_2d_from(self, grid: aa.type.Grid2DLike, **kwargs):
        ...
        deflection_y = (
            alpha_circ * np.sqrt(1 + ellip) * (grid.array[:, 0] / grid_radii)
        )
        deflection_x = (
            alpha_circ * np.sqrt(1 - ellip) * (grid.array[:, 1] / grid_radii)
        )
